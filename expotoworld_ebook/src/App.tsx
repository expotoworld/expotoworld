...
function UserMenu({ onLogout, token, ebookId }: { onLogout: () => void; token: string | null; ebookId: string }) {
...
function Shell({ children, status, lastSavedAt, toolbar, actionsRight, afterUserMenu, token, ebookId }: React.PropsWithChildren<{ status: string; lastSavedAt: Date | null; toolbar: React.ReactNode; actionsRight?: React.ReactNode; afterUserMenu?: React.ReactNode; token?: string | null; ebookId: string }>) {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Try to refresh using mutex-protected function
        // This prevents duplicate refresh calls and token accumulation
  ...
  const saveDraft = useCallback(async (json: any) => {
    ...
    markSaving()
    try {
      await axios.post(`${API_BASE}/api/ebook/draft/${ebookId}`, json, { headers: { Authorization: `Bearer ${token}` } })
      markSaved()
    } catch (e) {
      console.error(e)
  ...
  const editor = useEditor({
    extensions: [
      ...
      MediaDeletionExtension.configure({
        onDelete: (url: string) => {
          // Extract ebookId from URL pattern: https://assets.expotoworld.com/ebooks/{ebookId}/
          const match = url.match(/^https:\/\/assets\.expotoworld\.com\/ebooks\/([^/]+)\//)
          if (match) {
            const extractedEbookId = match[1]
            deleteMediaFromS3(url, extractedEbookId)
          }
        },
      }),
  ...
  useEffect(() => {
    if (!token || !editor || !ebookId) return
    let cancelled = false
    ;(async () => {
      try {
        const res = await axios.get(`${API_BASE}/api/ebook/draft/${ebookId}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <ThemeProvider>
      <Shell token={token}
        ...
        toolbar={<Toolbar editor={editor} ebookId={ebookId} outlineOpen={outlineOpen} onToggleOutline={() => setOutlineOpen(v => !v)} zoomLevel={zoomLevel} onZoomChange={setZoomLevel} onOpenHistory={() => setVersionsOpen(true)} onOpenDictionary={() => setDictOpen(true)} />}
        actionsRight={(
          <>
      ...
      >
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button className="primary-btn" onClick={async () => {
                ...
                try {
                  ...
                  await axios.post(`${API_BASE}/api/ebook/${ebookId}/versions`, { label }, { headers })
                  setSaveOpen(false)
        ...
        <VersionHistorySidebar open={versionsOpen} onClose={() => setVersionsOpen(false)} token={token!} ebookId={ebookId} onRestored={(content) => { if (content) editor?.commands.setContent(content, false) }} />
      </ThemeProvider>
    )
  }
