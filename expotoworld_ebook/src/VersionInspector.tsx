...
export default function VersionInspector() {
  const { ebookId, id } = useParams<{ ebookId: string; id: string }>()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!tok || !ebookId) { nav('/'); return }
        const res = await axios.get(`${API_BASE}/api/ebook/${ebookId}/versions/${id}/content`, { headers: { Authorization: `Bearer ${tok}` } })
