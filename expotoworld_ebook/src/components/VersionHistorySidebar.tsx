...
export default function VersionHistorySidebar({ open, onClose, token, ebookId, onRestored }: Props) {
  ...
  const load = async (k: 'manual'|'published') => {
    const res = await axios.get(`${API_BASE}/api/ebook/${ebookId}/versions?kind=${k}`, { headers })
    const list = (res.data?.items || []) as VersionItem[]
    if (k === 'manual') setManual(list)
    else setPublished(list)
  }

  useEffect(() => { if (open) { load('manual'); load('published') } }, [open, ebookId])

  const onInspect = (id: string) => {
    window.location.assign(`/ebook-editor/${ebookId}/versions/${id}`)
  }
  ...
  const doRestore = async (id: string) => {
    setBusyId(id)
    try {
      await axios.post(`${API_BASE}/api/ebook/${ebookId}/versions/${id}/restore`, null, { headers })
      const res = await axios.get(`${API_BASE}/api/ebook/draft/${ebookId}`, { headers })
  ...
  const doDelete = async (id: string) => {
    setBusyId(id)
    try {
      await axios.delete(`${API_BASE}/api/ebook/${ebookId}/versions/${id}`, { headers })
  ...
  const doPublish = async (id: string, label: string) => {
    setBusyId(id)
    try {
      await axios.post(`${API_BASE}/api/ebook/${ebookId}/versions/${id}/publish`, label ? { label } : {}, { headers })
      // Refresh both lists: manual stays unchanged, published gets the new version
      await load('manual'); await load('published')
    } finally { setBusyId(null) }
  }
  const doRename = async (id: string, label: string) => {
    setBusyId(id)
    try {
      await axios.patch(`${API_BASE}/api/ebook/${ebookId}/versions/${id}`, { label }, { headers })
      await load('manual'); await load('published')
