...
export default function Toolbar({ editor, ebookId, outlineOpen, onToggleOutline, zoomLevel, onZoomChange, onOpenHistory, onOpenDictionary }: { editor: Editor | null; ebookId: string; outlineOpen?: boolean; onToggleOutline?: () => void; zoomLevel: number; onZoomChange: (n: number) => void; onOpenHistory?: () => void; onOpenDictionary?: () => void }) {
    ...
    <div className="editor-toolbar" role="toolbar" aria-label="Formatting toolbar">
      ...
      <input id="_media_input" name="_media_input" type="file" accept="image/*,video/mp4,video/quicktime,audio/mpeg,audio/mp4,audio/x-m4a,audio/wav" style={{ display: 'none' }} onChange={async (e) => {
        ...
        try {
          ...
          const token = (() => { try { return JSON.parse(localStorage.getItem('ebook_token') || 'null')?.token || null } catch { return null } })();
          const response = await axios.post(`${API_BASE}/api/ebook/${ebookId}/media`, formData, {
