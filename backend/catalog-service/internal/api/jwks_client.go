...
func getJWKSURL() string {
	if v := os.Getenv("AUTH_JWKS_URL"); v != "" { return v }
	return "http://localhost:8081/.well-known/jwks.json"
}

func ensureJWKSLoaded() error {
	var err error
	jwksOnce.Do(func() {
		err = refreshJWKS()
	})
...
func refreshJWKS() error {
	...
	jwksCache.mu.Lock()
	jwksCache.keys = m
	jwksCache.expires = time.Now().Add(10 * time.Minute)
	jwksCache.mu.Unlock()
	return nil
}

func keyFuncFromJWKS(token *jwt.Token) (interface{}, error) {
	if _, ok := token.Method.(*jwt.SigningMethodRSA); !ok { return nil, jwt.ErrSignatureInvalid }
	if err := ensureJWKSLoaded(); err != nil { return nil, err }
	kid, _ := token.Header["kid"].(string)
	jwksCache.mu.RLock()
	pub := jwksCache.keys[kid]
	exp := jwksCache.expires
	jwksCache.mu.RUnlock()
	if pub == nil || time.Now().After(exp) {
		if err := refreshJWKS(); err != nil { return nil, err }
		jwksCache.mu.RLock()
		pub = jwksCache.keys[kid]
		jwksCache.mu.RUnlock()
	}
