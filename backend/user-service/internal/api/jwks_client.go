...
func getJWKSURL() string { if v := os.Getenv("AUTH_JWKS_URL"); v != "" { return v }; return "http://localhost:8081/.well-known/jwks.json" }

func refreshJWKS() error {
	resp, err := (&http.Client{Timeout: 5 * time.Second}).Get(getJWKSURL())
