...
const CategorySelector: React.FC<CategorySelectorProps> = ({
    ...
    const fetchCategories = async () => {
      try {
        ...
        try {
          ...
          setCategories(categoriesFromApi.length > 0 ? categoriesFromApi : mockCategories);
        } catch (apiError) {
  ...
  const handleChange = (event: any) => {
    ...
    if (multiple) {
      ...
      onCategoriesChange(selectedIds.map((id: string | number) => parseInt(id as string, 10)));
    } else {
      // For single selection
      onCategoriesChange(parseInt(String(value), 10));
    }
  };
  ...
  const renderValue = (selected: any) => {
    ...
    return (
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
        {selected.map((categoryId: number) => {
  ...
  if (loading) {
    return (
      <FormControl fullWidth disabled>
        ...
        <OutlinedInput label={label} endAdornment={<CircularProgress size={20} />} />
      </FormControl>
    );
  }
