...
import type { Organization, StorePartner, Store } from '../types/domain';

interface StorePartnersEditorProps {
  open: boolean;
...
const StorePartnersEditor: React.FC<StorePartnersEditorProps> = ({ open, onClose, store }) => {
  const [partners, setPartners] = useState<Organization[]>([]);
  const [rows, setRows] = useState<PartnerRow[]>([{ partner_org_id: '' }]);
  ...
  useEffect(() => {
    if (!open) setRows([{ partner_org_id: '' }]);
  }, [open]);

  const addRow = () => setRows(prev => [...prev, { partner_org_id: '' }]);
  const removeRow = (idx: number) => setRows(prev => prev.filter((_, i) => i !== idx));
  const updateRow = (idx: number, patch: Partial<PartnerRow>) =>
    setRows(prev => prev.map((r, i) => (i === idx ? { ...r, ...patch } : r)));

  const save = async () => {
    try {
      setSaving(true);
      const mappings: StorePartner[] = rows
        .filter(r => r.partner_org_id)
        .map(r => ({ partner_org_id: r.partner_org_id }));
      await relationshipService.manageStorePartners(store.id, mappings);
      onClose(true);
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Failed to save partners';
      alert(message);
    } finally {
      setSaving(false);
    }
