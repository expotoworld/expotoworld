...
import type { Organization, ProductLogisticsMapping, Product } from '../types/domain';

interface ProductLogisticsEditorProps {
  open: boolean;
...
const ProductLogisticsEditor: React.FC<ProductLogisticsEditorProps> = ({ open, onClose, product }) => {
  const [tpls, setTpls] = useState<Organization[]>([]);
  const [rows, setRows] = useState<LogisticsRow[]>([{ tpl_org_id: '' }]);
  ...
  const updateRow = (idx: number, patch: Partial<LogisticsRow>) =>
    setRows(prev => prev.map((r, i) => (i === idx ? { ...r, ...patch } : r)));

  const save = async () => {
    try {
      setSaving(true);
      const mappings: ProductLogisticsMapping[] = rows
        .filter(r => r.tpl_org_id)
        .map(r => ({ tpl_org_id: r.tpl_org_id }));
      await relationshipService.manageProductLogistics(product.id, mappings);
      onClose(true);
    } catch (e) {
      const message = e instanceof Error ? e.message : 'Failed to save logistics';
      alert(message);
    } finally {
      setSaving(false);
    }
