...
const DeleteProductDialog: React.FC<DeleteProductDialogProps> = ({ open, onClose, product, onProductDeleted }) => {
  const { showSuccess } = useToast();
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [deleteType, setDeleteType] = useState<'soft' | 'hard'>('soft'); // 'soft' or 'hard'
        ...
        <Box sx={{ textAlign: 'center', py: 2 }}>
          ...
          <Box sx={{ textAlign: 'left', mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 2 }}>
            ...
            <FormControl component="fieldset">
              <RadioGroup
                value={deleteType}
                onChange={(e: any) => setDeleteType(e.target.value as 'soft' | 'hard')}
              >
                <FormControlLabel
                  value="soft"
                  control={<Radio />}
