...
import type { Organization, ProductSourcingMapping, Region, Product } from '../types/domain';

interface ProductSourcingEditorProps {
  open: boolean;
...
const ProductSourcingEditor: React.FC<ProductSourcingEditorProps> = ({ open, onClose, product }) => {
  const [regions, setRegions] = useState<Region[]>([]);
  const [manufacturers, setManufacturers] = useState<Organization[]>([]);
  const [rows, setRows] = useState<SourcingRow[]>([{ region_id: '', manufacturer_org_id: '' }]);
  ...
  const updateRow = (idx: number, patch: Partial<SourcingRow>) =>
    setRows(prev => prev.map((r, i) => (i === idx ? { ...r, ...patch } : r)));

  const save = async () => {
    try {
      setSaving(true);
      const mappings: ProductSourcingMapping[] = rows
      ...
      const message = e instanceof Error ? e.message : 'Failed to save sourcing';
      alert(message);
    } finally {
      setSaving(false);
    }
