...
interface CategoryWithSubcategories {
  ...
  display_order?: number;
  is_active?: boolean;
  subcategories?: SubcategoryWithMeta[];
}
...
interface PreviewContext {
  mode: 'store' | 'category' | 'subcategory';
...
const CategoryListPage: React.FC = () => {
  ...
  const [editingSubcategory, setEditingSubcategory] = useState<SubcategoryWithMeta | null>(null);
  ...
  const fetchCategories = useCallback(async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        ...
        const categoriesArray = Array.isArray(data) ? (data as CategoryWithSubcategories[]) : [];
  ...
  const fetchStores = useCallback(async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Ensure data is always an array
        const storesArray: Store[] = Array.isArray(data) ? (data as Store[]) : [];
  ...
  const handleCreateSubcategory = async (): Promise<void> => {
    try {
      // Validate display order
      ...
      if (!selectedCategoryForSubcategory) {
        showToast('No category selected for subcategory', 'error');
        return;
      }

      let subcategoryData: SubcategoryFormState = { ...subcategoryForm };

      // If there's an image file, we'll upload it after creating the subcategory
      if (subcategoryImageFile) {
        subcategoryData = { ...subcategoryData, image_url: '' }; // Will be set after image upload
      }

      const response = await fetch(
        `${CATALOG_BASE}/categories/${selectedCategoryForSubcategory.id}/subcategories`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleUpdateSubcategory = async (): Promise<void> => {
    try {
      // Validate display order
      if (subcategoryForm.display_order < 1) {
        showToast('Display order must be at least 1', 'error');
      ...
      if (!editingSubcategory) {
        showToast('No subcategory selected for editing', 'error');
        return;
      }

      const subcategoryData: SubcategoryFormState = { ...subcategoryForm };
  ...
  const handleDeleteSubcategory = async (subcategoryId: number): Promise<void> => {
  ...
  const handleSubcategoryImageUpload = async (subcategoryId: number, file: File): Promise<void> => {
    try {
  ...
  const openSubcategoryDialogHandler = (
    category: CategoryWithSubcategories,
    subcategory: SubcategoryWithMeta | null = null,
  ): void => {
  ...
  const openCategoryDialog = (category: CategoryWithSubcategories | null = null): void => {
    if (category) {
                                ...
                                <IconButton size="small" component="label">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      handleSubcategoryImageUpload(subcategory.id, files[0]);
                                    }}
                                  />
                                </IconButton>
                              </Tooltip>
            ...
            >
              ...
              <input
                ...
                onChange={(e) => {
                  const { files } = e.target;
                  if (!files || !files[0]) return;
                  setSubcategoryImageFile(files[0]);
                }}
              />
            </Button>
