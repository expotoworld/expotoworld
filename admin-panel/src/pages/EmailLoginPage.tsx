...
const EmailLoginPage: React.FC = () => {
  ...
  const [step, setStep] = useState<number>(0); // 0: email, 1: verification code
  const [email, setEmail] = useState<string>(''); // Dynamic admin email input
  ...
  const handleVerifyCode = useCallback(async () => {
    ...
    try {
      ...
      // Redirect to dashboard (BrowserRouter)
