...
const emptyForm = (defaultType = ''): OrganizationFormState => ({
...
const OrganizationsPage: React.FC = () => {
  ...
  const [editing, setEditing] = useState<OrganizationOption | null>(null); // org or null
  const [form, setForm] = useState<OrganizationFormState>(emptyForm(''));
  const [errors, setErrors] = useState<Record<string, string | undefined>>({});
  const [brandOptions, setBrandOptions] = useState<OrganizationOption[]>([]);

  // User assignment state
  const [selectedUsers, setSelectedUsers] = useState<UserOption[]>([]); // [{ id, label, email, role }]
  const [userOptions, setUserOptions] = useState<UserOption[]>([]);
  const [userSearch, setUserSearch] = useState<string>('');
  const [loadingUserOptions, setLoadingUserOptions] = useState<boolean>(false);

  const [usersDialogOpen, setUsersDialogOpen] = useState<boolean>(false);
  const [usersDialogOrg, setUsersDialogOrg] = useState<OrganizationOption | null>(null);
  const [usersDialogUsers, setUsersDialogUsers] = useState<any[]>([]);

  const mapOrgTypeToUserRole = (t: string): string | null => {
  ...
  const [inlineUsersByOrg, setInlineUsersByOrg] = useState<Record<number, UserOption[]>>({}); // { [orgId]: [{id,label,email,role,orgRole}] }
  const [inlineLoading, setInlineLoading] = useState<boolean>(false);
  const [inlineSearch, setInlineSearch] = useState<string>('');
  const [inlineUserOptions, setInlineUserOptions] = useState<UserOption[]>([]);
  const [inlineLoadingOptions, setInlineLoadingOptions] = useState<boolean>(false);
  ...
  const loadMoreInline = useCallback(
    async (org: OrganizationOption) => {
      ...
      try {
        ...
        const opts: UserOption[] = (res.users || []).map((u: any) => ({
          id: u.id,
        ...
        console.error(e);
        setInlineUserOptions([]);
      } finally {
        setInlineLoadingOptions(false);
      }
    },
    [inlineSearch],
  );
  ...
  const load = useCallback(async () => {
    try {
      ...
      setOrgs((data?.organizations || []) as OrganizationOption[]);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    const h = setTimeout(async () => {
      try {
        ...
        const pageOpts: UserOption[] = (res.users || []).map((u: any) => ({
          id: u.id,
          label: `${u.full_name || u.username}${u.email ? ' (' + u.email + ')' : ''}`,
          email: u.email,
          role: u.role,
        ...
        setUserOptions((prev) =>
          userPage === 1
            ? pageOpts
            : [
                ...prev.filter((o) => !o.loadMore),
                ...pageOpts.filter((o1) => !prev.some((p) => p.id === o1.id)),
              ],
        );
        if ((res.users || []).length === 20) {
          setUserOptions((prev) => [
            ...prev.filter((o) => !o.loadMore),
            { id: '__load_more', label: 'Load more...', loadMore: true },
          ]);
        }
      } catch (e) {
        console.error('Failed to fetch users', e);
        if (userPage === 1) setUserOptions([]);
      } finally {
  ...
  useEffect(() => {
    const loadParents = async () => {
      ...
      try {
        ...
        setBrandOptions((res?.organizations || []) as OrganizationOption[]);
      } catch (e) {
  ...
  const openEdit = async (o: OrganizationOption) => {
    setEditing(o);
    setForm({
      org_type: o.org_type || '',
      name: o.name || '',
    ...
    try {
      ...
      const items: UserOption[] = (res || []).map((u: any) => ({
        id: String(u.user_id ?? u.id),
        label: `${u.full_name || 'Unnamed'}${u.email ? ' (' + u.email + ')' : ''}`,
        email: u.email,
        role: u.role,
        orgRole: u.org_role,
      }));
  ...
  const handleSave = async () => {
    try {
      const payload = {
        ...
        name: form.name.trim(),
        parent_org_id:
          form.parent_org_id === '' || form.parent_org_id === null
            ? null
            : Number(form.parent_org_id),
      ...
      let newOrgId: number | null = editing?.org_id ?? null;
      if (editing) {
      ...
      if (targetOrgId && form.org_type !== 'Brand') {
        const assignments = selectedUsers.map((u) => ({
          user_id: Number(u.id),
          primary_role: u.orgRole || 'Manager',
        }));
  ...
  const handleDelete = async (o: OrganizationOption) => {
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <FormControl sx={{ minWidth: 200 }} size="small">
            ...
            <Select label="Type" value={orgType} onChange={(e) => setOrgType(e.target.value as string)}>
                ...
                <TableRow key={o.org_id} hover>
                          ...
                          <Box sx={{ p: 2 }}>
                            {o.org_type === 'Brand' ? (
                              <Typography variant="body2" color="text.secondary">
                                Brand organizations cannot have users assigned.
                              </Typography>
                            ) : (
                              <Stack spacing={1}>
                                  ...
                                  <Box key={u.id} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                      ...
                                      <Select
                                        ...
                                        onChange={(e) => {
                                          ...
                                          setInlineUsersByOrg((prev) => ({
                                            ...prev,
                                            [o.org_id]: (prev[o.org_id] || []).map((x) =>
                                              x.id === u.id ? { ...x, orgRole: val } : x,
                                            ),
                                          }));
                                        }}
                                      >
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        setInlineUsersByOrg((prev) => ({
                                          ...prev,
                                          [o.org_id]: (prev[o.org_id] || []).filter((x) => x.id !== u.id),
                                        }))
                                      }
                                    >
                                      Remove
                                ...
                                <Autocomplete
                                  ...
                                  renderInput={(params) => (
                                    <TextField
                                      {...params}
                                      label={`Add ${mapOrgTypeToUserRole(o.org_type)} user`}
                                ...
                                <Stack direction="row" spacing={1}>
                                  <Button variant="outlined" onClick={() => setExpandedOrgId(null)}>
                                    Close
                                  </Button>
                                  <Button
                                    variant="contained"
                                    ...
                                    onClick={async () => {
                                      try {
                                        ...
                                        const assignments = (inlineUsersByOrg[o.org_id] || []).map((u) => ({
                                          user_id: Number(u.id),
                                          primary_role: u.orgRole || 'Manager',
                                        }));
                                        ...
                                        console.error(e);
                                        alert(e?.response?.data?.error || 'Failed to save users');
                                      } finally {
                  ...
                  <TableCell align="right">
                    <Tooltip title="View Users">
                      <IconButton size="small" onClick={async () => { try { const res = await orgService.getOrganizationUsers(o.org_id); setUsersDialogUsers(res || []); setUsersDialogOrg(o); setUsersDialogOpen(true);} catch(e){ console.error(e);} }}>
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                value={form.org_type}
                onChange={(e) => {
                  const nextType = e.target.value as string;
                  setForm({
                    ...form,
                    org_type: nextType,
                    parent_org_id: nextType === 'Brand' ? '' : form.parent_org_id,
                  });
              ...
              <FormControl fullWidth size="small" error={Boolean(errors.parent_org_id)}>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    setForm({ ...form, parent_org_id: val });
            ...
            {form.org_type && form.org_type !== 'Brand' ? (
              <Box>
                ...
                <Autocomplete
                  ...
                  options={userOptions}
                  value={selectedUsers}
                  onChange={(_e, val) => setSelectedUsers(val as UserOption[])}
                          ...
                          <Select
                            ...
                            onChange={(e) => {
                              ...
                              setSelectedUsers((prev) =>
                                prev.map((x) => (x.id === u.id ? { ...x, orgRole: val } : x)),
                              );
                            }}
                          >
                            <MenuItem value="Owner">Owner</MenuItem>
