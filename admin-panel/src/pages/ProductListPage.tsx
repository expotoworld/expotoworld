...
const toImg = (url: string | null | undefined): string => (url && !url.startsWith('http') ? `${API_BASE}${url}` : url || '');
...
const ProductListPage: React.FC = () => {
  ...
  const [previewCtx, setPreviewCtx] = useState<PreviewContext | null>(null); // { mode, entity }
  ...
  const handleProductUpdated = (updated: Product | null): void => {
  ...
  const handleStatusChanged = (productId: number, newStatus: boolean): void => {
  ...
  return (
    <Box>
      ...
      <ProductForm
        ...
        onClose={handleCloseModals}
        product={selectedProduct}
        onProductUpdated={(product) => handleProductUpdated(product ?? null)}
      />
