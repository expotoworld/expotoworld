...
const DashboardPage: React.FC = () => {
  ...
  const [roleStats, setRoleStats] = useState<DashboardRoleStats>({
    customers: 0,
    manufacturers: 0,
    partners3pl: 0,
    partners: 0,
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [prodRes, storeRes, orderRes, userRes] = results as [
          PromiseSettledResult<Product[]>,
          PromiseSettledResult<Store[]>,
          PromiseSettledResult<OrderStatistics>,
          PromiseSettledResult<UserAnalytics>,
        ];

        const productsData =
          prodRes.status === 'fulfilled' && Array.isArray(prodRes.value) ? prodRes.value : [];
        const storesData =
          storeRes.status === 'fulfilled' && Array.isArray(storeRes.value) ? storeRes.value : [];
        const orderStats =
          orderRes.status === 'fulfilled' && orderRes.value
            ? orderRes.value
            : { total_revenue: 0, total_orders: 0 };
        const userAnalytics =
          userRes.status === 'fulfilled' && userRes.value ? userRes.value : {};
        ...
        const byRole: UserAnalyticsByRole = userAnalytics.users_by_role || {};
        setRoleStats({
