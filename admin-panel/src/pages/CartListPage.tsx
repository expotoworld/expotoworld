...
import { CartDetails, CartSummary } from '../types/domain';
...
const CartListPage: React.FC = () => {
  ...
  const [selectedCart, setSelectedCart] = useState<CartDetails | null>(null);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState<boolean>(false);
  const [cartToDelete, setCartToDelete] = useState<CartSummary | null>(null);
  ...
  const handleFilterChange = (field: keyof CartFilters, value: string): void => {
    setFilters(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleViewCart = async (cart: CartSummary): Promise<void> => {
    try {
      const cartDetails = await cartService.getCart(String(cart.id));
  ...
  const handleDeleteCart = (cart: CartSummary): void => {
    setCartToDelete(cart);
    setDeleteConfirmOpen(true);
  };

  const confirmDeleteCart = async (): Promise<void> => {
    if (!cartToDelete) return;

    try {
      await cartService.deleteCart(String(cartToDelete.id));
              ...
              <TableBody>
                {carts.map((cart) => (
                  <TableRow key={cart.id} hover>
                    ...
                    <TableCell>
                      <Chip
                        label={miniAppTypeLabels[cart.mini_app_type as keyof typeof miniAppTypeLabels] || cart.mini_app_type}
                        size="small"
                        sx={{
                          backgroundColor:
                            miniAppTypeColors[cart.mini_app_type as keyof typeof miniAppTypeColors] || '#gray',
                          color: 'white',
                          fontWeight: 500,
                        }}
                      />
